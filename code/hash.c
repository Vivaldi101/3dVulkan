...
static void vk_pipeline_hash_function(vk_pipeline_hash_table* table, void(*p)(void* ctx, VkPipeline pipeline), void* ctx)
{
   ...
   while(count != table->count)
   {
      if(table->keys[index])
      {
         VkPipeline pipeline = table->values[index];
         p(ctx, pipeline);
         ++count;
      }
      index = (index + 1) % table->max_count;
...
static VkPipeline vk_pipeline_hash_lookup(vk_pipeline_hash_table* table, const char* key)
{
   u32 index = hash(key) % table->max_count;
   u32 old_index = index;
...
static void vk_pipeline_hash_insert(vk_pipeline_hash_table* table, const char* key, VkPipeline value)
{
   if(table->count == table->max_count)
      return;
   ...
   while(table->keys[index])
   {
      ...
      {
         const char* tmp_key = table->keys[index];
         VkPipeline tmp_value = table->values[index];
